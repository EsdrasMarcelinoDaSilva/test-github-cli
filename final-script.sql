...
CREATE TABLE reserva_quarto (
    ...
    CHECK (data_checkout > data_checkin) -- Garante que checkout seja após checkin
);
...
SELECT 
    ...
    q.preco_diaria
FROM 
    Hospede h  -- Aqui, começamos pela tabela Hospede
JOIN 
    Reserva r ON h.id = r.hospede_id  -- Conectamos com a tabela Reserva
JOIN 
    Reserva_Quarto rq ON r.id = rq.reserva_id  -- Conectamos com Reserva_Quarto
JOIN 
    Quarto q ON rq.quarto_id = q.id;  -- Conectamos com a tabela Quarto
...
SELECT 
    ...
    r.id AS reserva_id
FROM 
    Reserva_Quarto rq  -- Começamos pela tabela Reserva_Quarto
JOIN 
    Quarto q ON rq.quarto_id = q.id  -- Conectamos com Quarto
JOIN 
    Reserva r ON rq.reserva_id = r.id  -- Conectamos com Reserva
JOIN 
    Hospede h ON r.hospede_id = h.id;  -- Conectamos com Hospede
